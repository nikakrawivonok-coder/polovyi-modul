...
let suppressRemoteRenderUntil = 0;
...
const syncAdapter = {
  ...
  subscribe(){
    ...
    this.roomRef.on("value", snapshot => {
      ...
      if(shouldSuppressRemoteRender()){
        // V19.22.2: local field editing already changed the visible input.
        // Avoid full render echo from Firebase because it closes details and jumps scroll.
      } else {
        render();
      }
...
function enemyAttack(enemyId, mode){
  ...
  if(mutant){
    ...
    const resolved = rollEnemyWeaponDamage(enemy, hits, crits);
  ...
  setCombatBrief({
    ...
    targetDefenseBase: targetDefenseBaseForClarity,
    targetCovered: targetCoveredForClarity,
    attackMod: cfg.mod + suppressionPenalty,
    attrMod: 0,
    coverPenalty: burstCoverPenalty,
    recoilInfo: burstRecoilInfo,
    shooterDefenseBeforeExposure: exposureClarity.before,
...
function renderProfileGmQuickEdit(p){
  const box = qs("#profileGmQuickEdit");
  if(!box) return;
  if(appSession.role !== "gm"){
    box.innerHTML = "";
    box.hidden = true;
  ...
  box.innerHTML = `
    <div class="profile-edit-head">
      <span>Швидке редагування профілю Майстром</span>
      <small>тапай +/- без пошуку внизу панелі</small>
    </div>
    <div class="profile-edit-grid">
      <div class="profile-edit-line">
        <b>HP</b>
        <button class="micro-btn" data-player-step="${escapeAttr(pid)}" data-field="hp" data-delta="-1">−</button>
        <span>${escapeHtml(String(p.hp ?? "?"))}/${escapeHtml(String(p.hpMax ?? "?"))}</span>
        <button class="micro-btn" data-player-step="${escapeAttr(pid)}" data-field="hp" data-delta="1">+</button>
      </div>
      <div class="profile-edit-line">
        <b>Max HP</b>
        <button class="micro-btn" data-player-step="${escapeAttr(pid)}" data-field="hpMax" data-delta="-1">−</button>
        <span>${escapeHtml(String(p.hpMax ?? "?"))}</span>
        <button class="micro-btn" data-player-step="${escapeAttr(pid)}" data-field="hpMax" data-delta="1">+</button>
      </div>
      <div class="profile-edit-line">
        <b>Захист</b>
        <button class="micro-btn" data-player-step="${escapeAttr(pid)}" data-field="defense" data-delta="-1">−</button>
        <span>${escapeHtml(playerDefenseDisplay(p, true))}</span>
        <button class="micro-btn" data-player-step="${escapeAttr(pid)}" data-field="defense" data-delta="1">+</button>
      </div>
      <div class="profile-edit-line">
        <b>Max Зах.</b>
        <button class="micro-btn" data-player-step="${escapeAttr(pid)}" data-field="defenseMax" data-delta="-1">−</button>
        <span>${escapeHtml(String(p.defenseMax ?? p.defense ?? 12))}</span>
        <button class="micro-btn" data-player-step="${escapeAttr(pid)}" data-field="defenseMax" data-delta="1">+</button>
      </div>
      <div class="profile-edit-line">
        <b>Втома</b>
      ...
      <div class="profile-edit-line">
        <b>Зараж.</b>
        <button class="micro-btn" data-player-step="${escapeAttr(pid)}" data-field="infection" data-delta="-1">−</button>
        <span>${escapeHtml(String(p.infection ?? p.radiation ?? 0))}</span>
        <button class="micro-btn" data-player-step="${escapeAttr(pid)}" data-field="infection" data-delta="1">+</button>
      </div>
      <div class="profile-edit-line">
        <b>Набої</b>
        <button class="micro-btn" data-player-step="${escapeAttr(pid)}" data-field="ammo" data-delta="-1">−</button>
        <span>${escapeHtml(String(p.ammo ?? 0))}</span>
        <button class="micro-btn" data-player-step="${escapeAttr(pid)}" data-field="ammo" data-delta="1">+</button>
      </div>
    </div>
  `;
...
function gmEnemyFullEditor(enemy){
  ...
  enemy.stats = enemy.stats || {};
  normalizeCharacterWeapons(enemy);
  return `<details class="enemy-editor-panel" data-enemy-editor="${escapeAttr(enemy.id)}" ${expandedStateEnemyDetails[enemy.id] ? "open" : ""}>
...
function uniqueWeaponItemId(character, baseId){
  const base = String(baseId || "weapon").replace(/[^a-zA-Z0-9_-]+/g, "_") || "weapon";
  const used = new Set((character?.inventory || []).map(i => String(i.id || "")));
...
function createWeaponInventoryItem(weaponKey, overrides = {}){
  const key = weaponKey || "pm";
  const catalog = WEAPON_CATALOG[key] || WEAPON_CATALOG.pm;
  const range = overrides.range || (key === "obrez" ? "near" : "far");
  const profile = catalog[range] || catalog.far || WEAPON_CATALOG.pm.far;
  const bonus = Number(profile.bonus || 0);
  const damage = overrides.damage || `${profile.dice}${bonus ? (bonus > 0 ? "+" : "") + bonus : ""}`;
  return normalizeInventoryItem({
    id: overrides.id || key,
    type: "weapon",
    name: overrides.name || catalog.name || key,
    item: overrides.name || catalog.name || key,
    count: Number(overrides.count ?? 1),
    damage,
    range,
    ammoType: overrides.ammoType || "ammo",
    equipped: !!overrides.equipped,
    note: overrides.note || "додано до інвентарю"
  });
}

function addWeaponToCharacter(character, weaponKey, { makeActive = false } = {}){
  if(!character) return null;
  normalizeCharacterWeapons(character);
  const item = createWeaponInventoryItem(weaponKey, {
    id: uniqueWeaponItemId(character, weaponKey),
    equipped: makeActive
  });
  character.inventory.push(item);
  if(makeActive || !character.activeWeapon){
    setActiveWeaponForCharacter(character, item.id);
  }
  return item;
}

function setActiveWeaponForCharacter(character, weaponId){
  if(!character || !weaponId) return null;
  normalizeCharacterWeapons(character);
  const weapon = (character.inventory || []).find(i => i.type === "weapon" && i.id === weaponId);
  if(!weapon) return null;
  character.activeWeapon = weapon.id;
  character.weapon = WEAPON_CATALOG[weapon.id] ? weapon.id : (character.weapon || weapon.id);
  character.inventory.forEach(i => {
    if(i.type === "weapon") i.equipped = i.id === weapon.id;
  });
  if(weapon.range) character.range = weapon.range;
  return weapon;
}
...
function attackAccuracyParts({actionKey="", attackMod=0, attrMod=0, coverPenalty=0, recoilInfo=null} = {}){
  const key = actionKey || "";
  const parts = [];
  if(key === "shoot_aimed" || key === "aimed"){
    parts.push(`${signedMod(2)} за точний постріл`);
  } else if(key === "shoot_normal" || key === "normal"){
    parts.push(`${signedMod(-1)} за бойовий постріл`);
  } else if(key === "shoot_burst" || key === "burst"){
    const info = recoilInfo || (() => {
      const cover = Number(coverPenalty || 0);
      const recoilPenalty = Number(attackMod || 0) - cover;
      const step = Math.max(1, Math.round(Math.abs(recoilPenalty) / 2));
      return { step, penalty: recoilPenalty };
    })();
    const step = Number(info.step || 1);
    const penalty = Number(info.penalty || (-2 * step));
    parts.push(`${signedMod(penalty)} за ${uaQueueOrdinal(step)} чергу поспіль`);
    if(Number(coverPenalty || 0)) parts.push(`${signedMod(coverPenalty)} через ціль в укритті`);
  } else if(Number(attackMod || 0)){
    parts.push(`${signedMod(attackMod)} модифікатор атаки`);
  }
  if(Number(attrMod || 0)) parts.push(`${signedMod(attrMod)} від Точності`);
  return parts;
}
function accuracyBriefText({actionKey="", attackMod=0, attrMod=0, coverPenalty=0, recoilInfo=null} = {}){
  const total = Number(attackMod || 0) + Number(attrMod || 0);
  const parts = attackAccuracyParts({actionKey, attackMod, attrMod, coverPenalty, recoilInfo});
  if(!total && !parts.length) return "";
  return `точність ${signedMod(total)}${parts.length ? `: ${parts.join(", ")}` : ""}`;
}
function targetDefenseBriefLine({targetName="ціль", targetDefense=null, targetDefenseBase=null, targetCovered=false, targetIsPlayer=false} = {}){
  if(targetDefense === null || targetDefense === undefined || targetDefense === "") return "";
  const effective = Number(targetDefense);
  const base = targetDefenseBase === null || targetDefenseBase === undefined || targetDefenseBase === "" ? effective : Number(targetDefenseBase);
  const notes = [];
  if(targetCovered && effective >= base + 2) notes.push("+2 через укриття");
  const diff = effective - base;
  if(!notes.length && diff !== 0) notes.push(`${signedMod(diff)} через тимчасові ефекти`);
  return `Захист ${targetName}: ${effective}${notes.length ? ` (${notes.join(", ")})` : ""}`;
}
function shooterExposureLine({shooterName="Стрілець", before=null, after=null} = {}){
  if(before === null || before === undefined || after === null || after === undefined) return "";
  if(Number(before) === Number(after)) return "";
  return `${shooterName} розкрився: Захист ${shooterName} ${before} → ${after} до його наступного ходу.`;
}
...
function setCombatBrief(input = {}){
  ...
  const accText = accuracyBriefText({ actionKey, attackMod, attrMod, coverPenalty, recoilInfo });
  const diceLine = rollText ? `🎲 ${rollText}${totalsText && totalsText !== rollText ? ` → ${totalsText}` : ""}${accText ? ` (${accText})` : ""}` : "";
  const defenseLine = targetDefenseBriefLine({ targetName, targetDefense, targetDefenseBase, targetCovered, targetIsPlayer: targetType === "player" });
  ...
  const resultText = hits > 0 ? `${hits} влуч., завдана шкода: ${damage}` : "Промах";
  const ammoLine = shooterAmmo !== null && shooterAmmo !== undefined && shooterAmmo !== "" ? `Залишилося набоїв: ${shooterAmmo}.` : "";
  const exposureLine = shooterExposureLine({ shooterName, before: shooterDefenseBeforeExposure, after: shooterDefenseAfterExposure });
  const extraImportant = (Array.isArray(clarityLines) ? clarityLines : [])
    .filter(line => /Зброю заклинило|Критичний промах|Ціль вибула/.test(line));
...
const enemyStep = e.target.closest("[data-enemy-step]");
  ...
  if(activeWeaponBtn){
    const p = currentPlayer();
    const active = setActiveWeaponForCharacter(p, activeWeaponBtn.dataset.setActiveWeapon);
    save();
    render();
    showToast(active ? `Активна зброя: ${activeWeaponLabel(p)}.` : "Не вдалося обрати активну зброю.");
    return;
  }
...
document.addEventListener("input", e => {
  ...
  if(itemInput){
    ...
    if(field === "name") inv[idx].item = itemInput.value;
    normalizeCharacterWeapons(currentPlayer());
  }
});
...
document.addEventListener("change", e => {
  ...
  if(enemyAddWeapon){
    const enemy = findEnemyById(enemyAddWeapon.dataset.enemyAddWeapon);
    const weaponKey = enemyAddWeapon.value;
    if(enemy && weaponKey){
  ...
  const enemyActiveWeapon = e.target.closest("[data-enemy-active-weapon]");
  if(enemyActiveWeapon){
    const enemy = findEnemyById(enemyActiveWeapon.dataset.enemyActiveWeapon);
    if(enemy){
