...
Це функціональний checkpoint після `V19.27.1 — ROADMAP Attribute Checks Plan`.

## Важливо

У цій версії додано першу робочу систему Attribute Check Roller: Майстер створює перевірку характеристики, система рахує d20 з Умовою сцени, Втомою, Перевагою/Перешкодою, пише результат у журнал і дозволяє гравцю один перекид за +1 Втома.

## Поточна стабільна база перед цим checkpoint

Остання підтверджена стабільна база: `V19.27.1 — ROADMAP Attribute Checks Plan`, build/cache `19667`.
...
2. Перевірити, що відображається `V19.28 · 19668`.
...
```html
<link rel="stylesheet" href="./styles.css?v=19668">
<script src="./app.js?v=19668"></script>
```
